...
from vailodb_b.models  import Form, Form_Section, Form_Field, Form_FieldChoice


class FieldChoiceSerializer(serializers.ModelSerializer):
    class Meta:
        model = Form_FieldChoice
...
class FormSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for section_data in sections:
            ...
            section = Form_Section.objects.create(form=form, **section_data)
            # loop through all fields
            for field in fields:
                # extract choices from each field
                choices = field.pop("choices", [])
                # save the field data
                field = Form_Field.objects.create(section=section, **field)
                # loop through all choices and save the choice data
                for choice in choices:
                    Form_FieldChoice.objects.create(field=field, **choice)

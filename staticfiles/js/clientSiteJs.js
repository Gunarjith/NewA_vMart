...
function deleteProductCart(deleteId){
    ...
    for (const key in productCart) {
        ...
        if (key==deleteId){
            ...
            let comb=domainName+"/backProduct/"+String(deleteId)+"/"+cartTok+'/delete/'+'both'
            console.log(comb);
    ...
    for (const key in productCart) {
        ...
        if (key==decrId){
            ...
            // let pendingValue=productCart;
                ...
                let cartTok=localStorage.getItem('cart_token');
                let request = new XMLHttpRequest();
                let comb=domainName+"/backProduct/"+String(decrId)+"/"+cartTok+'/decrement/'+'both'
                console.log(comb);
                request.open("GET", comb, true);
                request.send()
    ...
    for (const key in productCart) {
        ...
        if (key==incrId){
            ...
            // let pendingValue=productCart;
                ...
                let cartTok=localStorage.getItem('cart_token');
                let request = new XMLHttpRequest();
                let comb=domainName+"/backProduct/"+String(incrId)+"/"+cartTok+'/increment/'+'both'
                console.log(comb);
                request.open("GET", comb, true);
                request.send();
...
let a=fetch(url).then(function(response) {
 ...
 console.log(data.product[0],"____productObject");
    ...
    cartDetails();
     ...
     let request = new XMLHttpRequest();
     let comb=domainName+"/backProduct/"+String(product__id)+"/"+cartTok+'/addTwoCart/'+'both'
     console.log(comb);
     request.open("GET", comb, true);
     request.send();
...
function removeAll(){
    ...
    tokk_n.href = "https://wa.me/917411722847?text=Cart ID : "+cartTok;
    let tokk_n1=document.querySelector("#link_to1   ");
    tokk_n1.href = "https://wa.me/917411722847?text=Cart ID : "+cartTok;
